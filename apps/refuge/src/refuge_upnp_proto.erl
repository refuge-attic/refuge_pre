...
%% @end
-spec parse_msearch_resp(string()) -> {ok, device,  refuge_types:upnp_device()}
                                    | {ok, service, refuge_types:upnp_service()}
                                    | {ok, uuid}
                                    | {error, _Reason}.
...
-spec parse_description(inet:ip_address(), string())
        -> {ok, [refuge_types:upnp_device()],
                [refuge_types:upnp_service()]} | {error, _Reason}.
...
-spec parse_device_desc(inet:ip_address(), term()) -> refuge_types:upnp_device().
...
%%      </s:Envelope>'''
%% @end
-spec build_ctl_msg(refuge_types:upnp_service(), string(), [{string(), string()}]) -> string().
...
%%      '''
%% @end
%% @todo See the explanation in ``refuge_upnp_httpd''.
