...
-spec register_upnp_entity(pid(), device | service,
                           refuge_types:upnp_device() |
                           refuge_types:upnp_service()) -> true.
register_upnp_entity(Pid, Cat, Entity) ->
    Id = refuge_upnp_entity:id(Cat, Entity),
...
lookup_upnp_entity(Cat, Entity) ->
    case ets:lookup(?TAB_UPNP, refuge_upnp_entity:id(Cat, Entity)) of
...
update_upnp_entity(Pid, Cat, Entity) ->
    EntityId = refuge_upnp_entity:id(Cat, Entity),
...
handle_info({'DOWN', Ref, _, _, _}, S) ->
    ...
    case Type of
        {upnp, Cat, Entity} ->
            EntityId = refuge_upnp_entity:id(Cat, Entity),
            refuge_upnp_entity:unsubscribe(Cat, Entity),
